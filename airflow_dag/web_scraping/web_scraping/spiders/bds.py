...
class BdsSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        raw_broker_name = response.xpath("//span[@class = 're__contact-name js_contact-name']/text()").get()
        raw_broker_rank = response.xpath("(//div[@class = 'is-certification'])[1]/text()").get()
        ...
        price_per_m2 = raw_price_per_m2.strip() if raw_price_per_m2 else None
        project_name = raw_project.strip() if raw_project else None
        address = ','.join((str(raw_address.strip()).split(","))[1:]) if raw_address else None
        price = raw_price.strip() if raw_price else None
        area = raw_area.strip() if raw_area else None
        bedroom = raw_bedroom.strip() if raw_bedroom else None
        bathroom = raw_bathroom.strip() if raw_bathroom else None
        posted_date = raw_posted_date.strip() if raw_posted_date else None
        expired_date = raw_expired_date.strip() if raw_expired_date else None
        status = raw_status.strip() if raw_status else None
        investor = raw_investor.strip() if raw_investor else None
        broker_name = raw_broker_name.strip() if raw_broker_name else None
        broker_rank = raw_broker_rank.strip() if raw_broker_rank else None
        project_area_range = raw_project_area_range.strip() if raw_project_area_range else None
        number_of_apartments = raw_number_of_apartments.strip() if raw_number_of_apartments else None
        number_of_buildings = raw_number_of_buildings.strip() if raw_number_of_buildings else None
